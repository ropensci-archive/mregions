...
highseas <- mr_geojson(key = "MarineRegions:high_seas")
...
```{r}
ecoregions_wkt <- mr_as_wkt(mr_geojson(key = "Ecoregions:ecoregions", read = TRUE))
...
leaflet() %>%
  ...
  addGeoJSON(geojson = highseas$features) %>%
  setView(30, 30, zoom = 5)
```


```{r}
mapview(ecoregions_wkt)
```
